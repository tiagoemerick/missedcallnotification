...
public class ToSendTabActivity extends Activity {
	...
	private OnItemClickListener optionsLogListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			AlertDialog.Builder builder = new AlertDialog.Builder(ToSendTabActivity.this);
			...
			items[2] = getApplicationContext().getString(R.string.list_option_sendnow);
			...
			builder.setItems(items, new DialogInterface.OnClickListener() {
				...
				private void resendNotification(MissedCallLog mclSelected) {
					...
					if (mclSelected.getNotificationType().compareTo(idMailType) == 0) {
						...
						ComponentName startService = getApplicationContext().startService(intentSMS);
	...
	private void loadListAdapter() {
		...
		List<MissedCallLog> logsToSendState = (List<MissedCallLog>) getMissedCallNotiDSLog().findAllToSendState();
	...
	private void createComponentsView() {
		listToSend = (ListView) findViewById(R.id.listToSend);
	}

	private void createComponentsListeners() {
		listToSend.setOnItemClickListener(optionsLogListener);
	}

	private ProgressDialog createProgressDialog() {
		final ProgressDialog dialog = ProgressDialog.show(ToSendTabActivity.this, getApplicationContext().getString(R.string.app_name), getApplicationContext().getString(R.string.loading_label), false, false);
